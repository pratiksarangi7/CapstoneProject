...
namespace CapstoneProjectAPI.Services
{
    public class AuditLogService
    {
        ...
        public async Task<PagedResult<AuditLogResponseDto>> GetAuditLogsByActions(List<AuditAction> actions, int? userId = null, int pageNumber = 1, int pageSize = 10)
        {
            if (pageNumber < 1) pageNumber = 1;
            if (pageSize < 0 || pageSize > 100) pageSize = 10;
            ...
            {
                query = query.Where(al => actions.Contains(al.Action));
            }
            if (userId.HasValue)
            {
                query = query.Where(al => al.PerformedByUserId == userId);
